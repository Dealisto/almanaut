...
func newTestServerDB(t *testing.T) (http.Handler, *sql.DB) {
	...
	srv := New(Config{
		...
		Relationships: store.NewRelationshipRepo(db), Tags: store.NewTagRepo(db), VLANs: store.NewVLANRepo(db), DB: db,
...
func newTestServerProxmoxRepos(t *testing.T, pve proxmoxScanner, opts ProxmoxOptions) (http.Handler, *store.HostRepo, *store.RelationshipRepo) {
	...
	srv := New(Config{
		...
		Sites: store.NewSiteRepo(db), Locations: store.NewLocationRepo(db), Racks: store.NewRackRepo(db),
		Contacts:      store.NewContactRepo(db),
		Relationships: rels, Tags: store.NewTagRepo(db), VLANs: store.NewVLANRepo(db), DB: db,
		Logger: log.New(io.Discard, "", 0),
