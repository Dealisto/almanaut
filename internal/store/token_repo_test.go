...
func TestTokenUserByToken(t *testing.T) {
	...
	if _, err := tokens.UserByToken("nope"); !errors.Is(err, ErrNotFound) {
...
func TestTokenDeleteOwnerScoped(t *testing.T) {
	...
	if _, err := tokens.UserByToken("a1"); err != nil {
	...
	if _, err := tokens.UserByToken("a1"); !errors.Is(err, ErrNotFound) {
